...
using Pose6DMsg = shopee_interfaces::msg::Pose6D;
using PackeeVisionDetectProductsInCart = shopee_interfaces::srv::PackeeVisionDetectProductsInCart;
...
geometry_msgs::msg::PoseStamped toPoseStamped(const Pose6DMsg& pose_source, const std::string& frame_id) {
  geometry_msgs::msg::PoseStamped pose;
  pose.header.frame_id = frame_id;
  pose.pose.position.x = pose_source.x;
  pose.pose.position.y = pose_source.y;
  pose.pose.position.z = pose_source.z;

  tf2::Quaternion orientation;
  orientation.setRPY(pose_source.rx, pose_source.ry, pose_source.rz);
  pose.pose.orientation = tf2::toMsg(orientation);
  return pose;
}
...
public:
  ...
  : rclcpp::Node("packee_mtc_orchestrator", options) {
    ...
    gripper_close_pose_ = declare_string("gripper_close_pose", "closed", "Named pose representing a closed gripper.");
    ...
    vision_service_name_ = declare_string(
      ...
      "Service used to request Packee vision detection data.");
    ...
    vision_client_ = vision_proxy_node_->create_client<PackeeVisionDetectProductsInCart>(vision_service_name_);
...
private:
  ...
  void processSequences(PackeeMainStartMTC::Request request) {
    ...
    bool gripper_initialised = false;
    ...
    try {
      for (std::size_t index = 0; index < sequences.size(); ++index) {
        ...
        if (!gripper_initialised) {
          std::string gripper_prep_message;
          if (!planAndExecuteGripperNamed(gripper_open_pose_, gripper_prep_message)) {
            result.success = false;
            result.message = "Failed to ensure gripper open state: " + gripper_prep_message;
        ...
        pauseBeforeStage(pause_before_vision_request_, "before requesting vision coordinates");

        std::string vision_message;
        std::int32_t total_detected = 0;
        Pose6DMsg detected_pose;

        if (!delegateToVision(request, sequence, vision_message, total_detected, detected_pose)) {
          result.success = false;
        ...
        RCLCPP_INFO(this->get_logger(),
                    "Sequence %d (product %d) vision response: %s (detected=%d, pose=[%.3f, %.3f, %.3f])",
                    sequence.seq, sequence.id, vision_message.c_str(), total_detected,
                    detected_pose.x, detected_pose.y, detected_pose.z);

        std::string execution_message;
        if (!planAndExecutePickAndPlace(detected_pose, sequence, execution_message)) {
          result.success = false;
        ...
        std::ostringstream composed_message;
        composed_message << "Vision: " << vision_message
                         << " | Exec: " << execution_message;

        result.success = true;
        result.message = composed_message.str();
        result.total_detected = total_detected;
        results.push_back(result);
        pauseAfterSequence(sequence.seq);
      }
  ...
  bool planAndExecutePickAndPlace(const Pose6DMsg& detected_pose,
                                  const SequenceMsg& place_sequence,
                                  std::string& message) {
    std::string pick_message;
    if (!planAndExecutePickRoutine(detected_pose, pick_message)) {
      message = std::string("Pick failed: ") + pick_message;
      return false;
    }

    std::string place_message;
    if (!planAndExecuteSequence(place_sequence, place_message)) {
      message = std::string("Place failed: ") + place_message;
      return false;
    }

    std::ostringstream oss;
    oss << "Pick => " << pick_message << " | Place => " << place_message;
    message = oss.str();
    return true;
  }
  ...
  bool planAndExecutePickRoutine(const Pose6DMsg& detected_pose, std::string& message) {
    ...
    approach_pose.pose.position.z += approach_offset;

    pauseBeforeStage(pause_before_pick_approach_, "before pick approach motion");
    ...
    pauseBeforeStage(pause_before_pick_descent_, "before pick descent motion");
    ...
    pauseBeforeStage(pause_before_pick_grip_close_, "before gripper close");

    std::string gripper_close_message;
    if (!planAndExecuteGripperNamed(gripper_close_pose_, gripper_close_message)) {
      message = std::string("Gripper close failed: ") + gripper_close_message;
      return false;
    }
    pauseBeforeStage(pause_before_pick_lift_, "before pick lift motion");

    std::string lift_message;
    if (!planAndExecuteArmPose(approach_pose, lift_message)) {
      message = std::string("Lift failed: ") + lift_message;
      return false;
    }

    pauseBeforeStage(pause_before_home_, "before returning home with grasped object");
    ...
    oss << "Approach: " << approach_message
        ...
        << " | GripClose: " << gripper_close_message
        << " | Lift: " << lift_message
        << " | ReturnHome: " << home_message;
    message = oss.str();
    return true;
  }
  ...
  bool planAndExecuteSequence(const SequenceMsg& sequence, std::string& message) {
    ...
    if (!planAndExecuteArmPose(approach_pose, retreat_message)) {
  ...
  bool delegateToVision(const PackeeMainStartMTC::Request& request,
    ...
    auto service_request = std::make_shared<PackeeVisionDetectProductsInCart::Request>();
  ...
  std::int64_t service_wait_timeout_ms_{ 6000 };
  std::int64_t monitor_interval_ms_{ 500 };
  std::int64_t state_stale_timeout_ms_{ 2000 };
  std::chrono::milliseconds pause_before_vision_request_{ 5000 };
  std::chrono::milliseconds pause_before_pick_approach_{ 5000 };
  std::chrono::milliseconds pause_before_pick_descent_{ 5000 };
  std::chrono::milliseconds pause_before_pick_grip_close_{ 5000 };
  std::chrono::milliseconds pause_before_pick_lift_{ 5000 };
  std::chrono::milliseconds pause_before_home_{ 5000 };
  std::chrono::milliseconds pause_before_place_move_{ 5000 };
  std::chrono::milliseconds pause_before_descent_{ 5000 };
  std::chrono::milliseconds pause_before_release_{ 5000 };
  std::chrono::milliseconds pause_before_retreat_{ 5000 };
  std::chrono::milliseconds pause_between_sequences_{ 5000 };
  ...
  rclcpp::Client<PackeeVisionDetectProductsInCart>::SharedPtr vision_client_;
