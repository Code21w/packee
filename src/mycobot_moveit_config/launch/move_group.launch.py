...
def generate_launch_description():
    ...
    def configure_setup(context):
        ...
        # Paths within the package share
        ...
        joint_limits_file_rel_path = config_rel_path / 'joint_limits.yaml'
        kinematics_file_rel_path = config_rel_path / 'kinematics.yaml'
        moveit_controllers_file_rel_path = config_rel_path / 'moveit_controllers.yaml'
        srdf_model_rel_path = config_rel_path / f'{robot_name_str}.srdf'
        pilz_cartesian_limits_file_rel_path = config_rel_path / 'pilz_cartesian_limits.yaml'

        # Create MoveIt configuration
        moveit_config = (
            MoveItConfigsBuilder(robot_name_str, package_name=package_name_moveit_config)
            .trajectory_execution(file_path=moveit_controllers_file_rel_path)
            .robot_description_semantic(file_path=srdf_model_rel_path)
            .joint_limits(file_path=joint_limits_file_rel_path)
            .robot_description_kinematics(file_path=kinematics_file_rel_path)
            .planning_pipelines(
            ...
            .pilz_cartesian_limits(file_path=pilz_cartesian_limits_file_rel_path)
            .to_moveit_configs()
        )
